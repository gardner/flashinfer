...
def test_gdn_prefill_sm121_l2norms_q_and_k_when_enabled(monkeypatch):
    import flashinfer.gdn_prefill as gdn_prefill
    ...
    q, k, v, g, beta, cu_seqlens, output, output_state = _make_gdn_prefill_tensors()
    q.copy_(torch.randn_like(q).add_(1.0))
    k.copy_(torch.randn_like(k).add_(1.0))
    original_q = q.clone()
    original_k = k.clone()
    ...
    torch.testing.assert_close(
        ...
        torch.ones_like(torch.linalg.vector_norm(calls[0]["q"].float(), dim=-1)),
        atol=5e-3,
        rtol=5e-3,
    )
    torch.testing.assert_close(
        torch.linalg.vector_norm(calls[0]["k"].float(), dim=-1),
        torch.ones_like(torch.linalg.vector_norm(calls[0]["k"].float(), dim=-1)),
        atol=5e-3,
        rtol=5e-3,
    )
